...
namespace AccesoADatos
{
    public class ConexionPlatillos
    {
        ...
        {
            return Comando(string.Format("insert into Menu values(null,'{0}','{1}',{2},'{3}')",
        ...
        {
            return Comando(string.Format("delete from Menu where IdPlatillo = '{0}'", ep.IdPlatillo));
        }

        public string ActualizarPlatillo(EntidadPlatillos ep)
        {
            return Comando(string.Format("update Menu set Nombre = '{0}', Descripcion = '{1}', Precio = {2}, Imagen = '{3}' where IdPlatillo = {4}",
                ep.Nombre, ep.Descripcion, ep.Precio, ep.Imagen, ep.IdPlatillo));
        }

        public DataSet Mostrar(string nombre)
        {
            return Mostrar(string.Format("select p.IdPlatillo as 'ID', p.Nombre as 'Platillo', p.Descripcion as 'Descripción', p.Precio, p.Imagen from Menu p where p.Nombre like '%{0}%'", nombre), "menu");
        }
    }
}
